...
export const getUserById = async (req: Request, res: Response) => {
  try {
    ...
    const authUser = (req as any).user; // Desde authMiddleware
...
export const updateUser = async (req: Request, res: Response) => {
  try {
    ...
    const updatedUser = await UsuarioModel.update(
      ...
      address,
      phone,
      req.body.profilePhoto // Opcional: permitir actualizar vía JSON
    );
...
export const updateUserByAdmin = async (req: Request, res: Response) => {
  try {
    ...
    // 1. Intentar encontrar usuario en CuidadorModel
    ...
    if (cuidador) {
      ...
      const updatedCuidador = await CuidadorModel.update(
        ...
        cuidador.telefono // Mantener teléfono existente
      );
    ...
    // 2. Intentar encontrar usuario en UsuarioModel (Paciente)
...
export const deleteUser = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const role = (req.query.role as string) || "user"; // 'user' o 'caregiver'

    if (role === "admin") {
      // Prevenir borrar admins vía este endpoint genérico por seguridad, aunque la lógica lo permite
    ...
    if (role === "caregiver") {
      ...
      // Por defecto borrar un paciente (usuario)
...
export const assignDevice = async (req: Request, res: Response) => {
  try {
    ...
    // 2. Verificar si el dispositivo ya está asignado
    ...
    // 3. Asignar dispositivo al usuario
...
export const exportUsersCSV = async (req: Request, res: Response) => {
  try {
    ...
    // 1. Obtener Datos basados en Rol y Alcance
    ...
    // Si NO es scope='me', mantenemos la lógica original de exportación masiva para Admins/Cuidadores
      ...
      // Obtener datos enriquecidos (info dispositivo) para cada usuario asignado
      ...
      // Paciente - obtener sus propios datos (fallback por defecto)
